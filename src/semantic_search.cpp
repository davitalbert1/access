...
namespace semantic {
    ...
    static std::vector<std::string> split_into_chunks( const std::string& text, size_t chunk_size = 700, size_t overlap = 120) {
    ...
    void index_project(const std::string& root) {
        ...
        for (auto& f : list["files"]) {
            ...
            auto chunks = split_into_chunks(file_text);
            ...
            for (size_t i = 0; i < chunks.size(); i++) {
                ...
                if (e.embedding.empty()) {
                    std::cerr << "[Embedding] Failed to embed: " << e.path << " chunk " << i << std::endl;
                    continue;
                }
    ...
    std::string retrieve_context(const std::string& query) {
        ...
        std::string ctx;
        int chars_used = 0;
        const int MAX_CTX_CHARS = 2500; // Limit total context chars to save tokens

        for (int i = 0; i < 6 && i < (int)scored.size(); i++) {
            // Skip low-relevance results (below 0.5 cosine similarity)
            if (scored[i].first < 0.5f && i > 0) continue;

            std::string header = "FILE: " + scored[i].second->path + " CHUNK " + std::to_string(scored[i].second->chunk_index) + "\n";
            std::string body;
